        ...
        if (selectedAgent) {
          ...
          for (const stage of stages) {
            const res = await fetch(
              `${BACKEND_BASE_URL}/api/deals/stage/${stage}`,
              { headers }
            );
            if (res.ok) {
    ...
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <p style={styles.subtitle}>
          {user.role === "BUYER" &&
            "Track properties you're interested in purchasing"}
      ...
      {user.role === "ADMIN" && (
        <div style={styles.agentSelector}>
          ...
          <select
            value={selectedAgent || ""}
            onChange={(e) =>
              setSelectedAgent(e.target.value ? parseInt(e.target.value) : null)
            }
      ...
      <div style={styles.filterTabs}>
        ...
        >
          📈 Active (
          {
            deals.filter((d) => (d.stage || d.currentStage) !== "COMPLETED")
              .length
          }
          )
        </button>
