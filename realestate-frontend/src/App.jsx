...
function App() {
  ...
  // Fetch all properties on initial component load
  useEffect(() => {
  ...
  // Function to search properties by city
    ...
    <div style={{ fontFamily: 'Inter, system-ui, Arial', padding: 24, maxWidth: 1100, margin: '0 auto', color: '#111827' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <nav style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
          ...
          <span>Rent</span>
          <span>Sell</span>
          <button style={{ background: '#3b82f6', color: 'white', padding: '8px 14px', borderRadius: 10, border: 'none', cursor: 'pointer' }}>Login ▾</button>
        </nav>
      </header>
      ...
      <form onSubmit={handleSearchSubmit} style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 24 }}>
        ...
        />
        <button onClick={onSearch} style={{ background: '#3b82f6', color: 'white', padding: '18px 24px', borderRadius: 10, border: 'none', cursor: 'pointer', fontSize: 18 }}>Search</button>
      </form>
