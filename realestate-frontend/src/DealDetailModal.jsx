...
const DealDetailModal = ({ deal: initialDeal, onClose, onUpdate, userRole, showOnlyOverview = false }) => {
      ...
      const fetchFullDeal = async () => {
        try {
          const response = await fetch(
            `http://localhost:8080/api/deals/my-deals?userRole=${userRole}&userId=${initialDeal.agentId || initialDeal.buyerId || initialDeal.sellerId}`,
            {
              headers: {
          ...
          if (response.ok && data.data) {
            ...
            const foundDeal = data.data.find(d => d.dealId === initialDeal.dealId);
            if (foundDeal) {
              setDeal(foundDeal);
              console.log('✅ Fetched full deal with timeline dates:', foundDeal);
  ...
  const handleUpdateStage = async () => {
    if (!visitDate || !notes.trim()) {
      alert('❌ Please provide both the Date of Visit/Update and a Matter of Visit/Note.');
      return;
    }
...
const styles = {
  backdrop: {
    ...
    zIndex: 10001, overflowY: 'auto', padding: '20px'
  },
  modal: {
    backgroundColor: 'white', borderRadius: '16px', width: '100%', maxWidth: '1000px',
    maxHeight: '90vh', overflowY: 'auto', position: 'relative',
    boxShadow: '0 25px 50px rgba(0,0,0,0.3)', padding: '32px'
  },
  closeBtn: {
    position: 'absolute', top: '16px', right: '16px',
    background: 'none', border: 'none', fontSize: '32px', cursor: 'pointer',
    color: '#6b7280', padding: 0, width: '40px', height: '40px'
  },
  header: {
  ...
  notesBox: {
    ...
    border: '1px solid #e2e8f0', fontSize: '14px', color: '#475569', lineHeight: '1.6'
  },
  datesGrid: {
  ...
  // Timeline Styles
  timelineContainer: {
    borderLeft: '3px solid #e2e8f0', paddingLeft: '20px', marginTop: '16px',
    marginBottom: '16px', maxHeight: '400px', overflowY: 'auto'
  },
  timelineEntry: {
  ...
  timelinePin: {
    ...
    height: '12px', width: '12px', backgroundColor: '#3b82f6',
  ...
  actionTitle: { fontSize: '16px', fontWeight: '700', color: '#1e293b', margin: '0 0 16px 0' },
  ...
  // Alerts
  successAlert: {
    padding: '20px', backgroundColor: '#ecfdf5', borderRadius: '8px',
    border: '1px solid #bbf7d0', textAlign: 'center', fontWeight: '600',
    color: '#059669', marginBottom: '16px'
  },
  warningAlert: {
    padding: '20px', backgroundColor: '#fef3c7', borderRadius: '8px',
    border: '1px solid #fde68a', textAlign: 'center', fontWeight: '600',
    color: '#b45309', marginBottom: '16px'
  },
  ...
  successBtn: {
    ...
    border: 'none', borderRadius: '6px', fontWeight: '600', cursor: 'pointer', marginBottom: '16px'
  },
  warningBtn: {
    padding: '10px 18px', backgroundColor: '#f59e0b', color: 'white',
    border: 'none', borderRadius: '6px', fontWeight: '600', cursor: 'pointer', marginBottom: '16px'
  },
  secondaryBtn: {
    width: '100%', padding: '10px 18px', backgroundColor: '#8b5cf6', color: 'white',
    border: 'none', borderRadius: '6px', fontWeight: '600', cursor: 'pointer'
  },
  ...
  viewPropertyBtn: {
    flex: 1, padding: '12px 20px', backgroundColor: '#10b981', color: 'white',
    border: 'none', borderRadius: '8px', fontWeight: '600', cursor: 'pointer',
    fontSize: '14px', transition: 'background 0.2s'
  },
  closeBottomBtn: {
    flex: 1, padding: '12px 20px', backgroundColor: '#6b7280', color: 'white',
    border: 'none', borderRadius: '8px', fontWeight: '600', cursor: 'pointer',
    fontSize: '14px', transition: 'background 0.2s'
  }
};
