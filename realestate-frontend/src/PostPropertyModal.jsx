...
function PostPropertyModal({ onClose, onPropertyPosted }) {
    ...
    const loadAreas = async () => {
        ...
        // The convertToIndianWords function was incorrectly here. It has been moved.
    };

    // === BUG FIX 1: The function is now moved here, in the correct scope ===
    ...
    const handleChange = (e) => {
        ...
        // This line causes an error because convertToIndianWords is not visible here
        // if (name === 'city') { setFormData(prev => ({ ...prev, city: processedValue, areaId: '' })); }
            ...
            <div style={styles.modal} onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} style={styles.form}>
                        ...
                        {!formData.imageUrl ? (
                            <>
                                <input type="file" accept="image/*" onChange={handleImageUpload} disabled={imageUploading}
                                    style={{
                                        display: 'block',
                                        width: '100%', // <-- ALIGNMENT FIX 2
                                        boxSizing: 'border-box', // <-- ALIGNMENT FIX 2
                            ...
                            <div style={{ textAlign: 'center' }}>
                                ...
                                <button type="button" onClick={() => setFormData(p => ({ ...p, imageUrl: '' }))} style={{ marginTop: '12px', padding: '10px 20px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>🗑️ Remove</button>
                            </div>
                        )}
                    </div>
                    ...
                    <div style={styles.row3}>
                        <div style={styles.field}><label style={styles.label}>🛏️ Bedrooms *</label><input type="number" name="bedrooms" value={formData.bedrooms} onChange={handleChange} min="0" max="20" style={styles.input} placeholder="2" required /></div>
                        <div style={styles.field}><label style={styles.label}>🚿 Bathrooms *</label><input type="number" name="bathrooms" value={formData.bathrooms} onChange={handleChange} min="0" max="20" style={styles.input} placeholder="2" required /></div>
                    ...
                    <div style={styles.row}>
                        <div style={styles.field}><label style={styles.label}>📐 Area (sqft)</label><input type="number" name="areaSqft" value={formData.areaSqft} onChange={handleChange} placeholder="1200" style={styles.input} /></div>
                        <div style={styles.field}>
                            <label style={styles.label}>💰 Expected Price (₹) *</label>
                            <input type="number" name="price" value={formData.price} onChange={handleChange} placeholder="e.g..,5000000" style={styles.input} required />
                            {priceInWords && <p style={styles.priceInWords}>{priceInWords}</p>}
                        </div>
                    </div>

                    <div style={styles.field}><label style={styles.label}>✨ Amenities (comma-separated)</label><input type="text" name="amenities" value={formData.amenities} onChange={handleChange} placeholder="Parking, Gym, Swimming Pool" style={styles.input} /></div>
