...
const AdminAgentsPage = () => {
  ...
  const fetchAllAgents = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:8080/api/users/agents', {
        headers: {
      ...
      if (response.ok) {
        ...
        const agentsList = data.success ? (data.data || []) : (Array.isArray(data) ? data : []);
  ...
  const fetchAgentDeals = async (agentId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/deals/admin/agent/${agentId}?userId=${user.id}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
        }
      });
  ...
  const handleSaveAgent = async () => {
    try {
      const response = await fetch(`http://localhost:8080/api/users/${editingAgent.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        filterAgents(agents.map(a => a.id === editingAgent.id ? updatedAgent : a), searchTerm);
  ...
  const handleDeleteAgent = async (agentId) => {
    if (window.confirm('Are you sure you want to delete this agent?')) {
      try {
        const response = await fetch(`http://localhost:8080/api/users/${agentId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('authToken')}`
          }
        ...
        if (response.ok) {
          ...
          filterAgents(agents.filter(a => a.id !== agentId), searchTerm);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <p style={styles.subtitle}>Manage all agents - view, edit, and modify agent details and their deals</p>
      </div>
            ...
            <div style={styles.agentsGrid}>
              {filteredAgents.map(agent => (
                <div
                  ...
                  style={{
                    ...styles.agentCard,
                    backgroundColor: selectedAgent?.id === agent.id ? '#dbeafe' : 'white',
                    borderColor: selectedAgent?.id === agent.id ? '#3b82f6' : '#e2e8f0'
                  }}
                  onClick={() => handleSelectAgent(agent)}
                >
                  <div style={styles.agentCardHeader}>
                    <div>
                      <h4 style={styles.agentName}>{agent.firstName} {agent.lastName}</h4>
          ...
          {selectedAgent ? (
            <>
              <div style={styles.detailsCard}>
                ...
                <div style={styles.detailsGrid}>
                  ...
                  <div>
                    ...
                    <p style={styles.value}>{selectedAgent.mobileNumber || 'N/A'}</p>
                  </div>
                  <div>
                    <p style={styles.label}>Username</p>
                    <p style={styles.value}>{selectedAgent.username || 'N/A'}</p>
                  </div>
                  <div>
                    <p style={styles.label}>Member Since</p>
                    <p style={styles.value}>{new Date(selectedAgent.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
                ...
                <div style={styles.statusSection}>
                  ...
                  <span style={{
                    ...styles.statusBadge,
                    backgroundColor: selectedAgent.isActive ? '#10b981' : '#ef4444'
                  }}>
              ...
              <div style={styles.dealsSection}>
                <h3 style={styles.dealsTitle}>Deals by {selectedAgent.firstName} ({agentDeals.length})</h3>
          ...
          <div style={styles.modalContent}>
            ...
            <div style={styles.formGroup}>
              ...
              <input
                ...
                onChange={(e) => setEditingAgent({...editingAgent, mobileNumber: e.target.value})}
                style={styles.input}
              />
            </div>
            ...
            <div style={styles.formGroup}>
              ...
              <select
                ...
                onChange={(e) => setEditingAgent({...editingAgent, isActive: e.target.value === 'true'})}
                style={styles.input}
              >
            ...
            <div style={styles.modalButtons}>
              <button onClick={handleSaveAgent} style={styles.saveBtn}>💾 Save Changes</button>
