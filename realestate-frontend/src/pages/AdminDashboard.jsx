...
const AdminDashboard = () => {
  ...
  const fetchAgents = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Calculate stats
        const totalDeals = agentsList.reduce((sum, a) => sum + (a.totalDeals || 0), 0);
        const completedDeals = agentsList.reduce((sum, a) => sum + (a.completedDeals || 0), 0);
  ...
  const fetchAgentDeals = async (agentId) => {
    try {
      const response = await fetch(`http://localhost:8080/api/deals/admin/agent/${agentId}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  if (loading) {
    return <div style={{ ...containerStyle, textAlign: 'center', paddingTop: '80px' }}>Loading agents...</div>;
  }
  ...
  return (
    <div style={containerStyle}>
      <div style={headerStyle}>
        ...
        <p style={{ color: '#64748b', margin: '0', fontSize: '16px' }}>Manage agents and monitor all deals</p>
      </div>
      ...
      <div style={statsGridStyle}>
        <div style={statCardStyle}>
          ...
          <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '4px' }}>Total Deals</div>
          <div style={{ fontSize: '28px', fontWeight: '700', color: '#1e293b' }}>{stats.totalDeals || 0}</div>
        </div>
        <div style={statCardStyle}>
          <div style={{ fontSize: '32px', marginBottom: '8px' }}>✅</div>
          <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '4px' }}>Completed</div>
          <div style={{ fontSize: '28px', fontWeight: '700', color: '#10b981' }}>{stats.completedDeals || 0}</div>
        </div>
        <div style={statCardStyle}>
          <div style={{ fontSize: '32px', marginBottom: '8px' }}>👥</div>
          <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '4px' }}>Active Agents</div>
          <div style={{ fontSize: '28px', fontWeight: '700', color: '#1e293b' }}>{agents.length}</div>
        </div>
      </div>
      ...
      <div style={gridStyle}>
        ...
        <div style={agentListStyle}>
          <div style={{ padding: '16px', backgroundColor: '#f8fafc', borderBottom: '1px solid #e5e7eb', fontWeight: '700' }}>
          ...
          {agents.map(agent => (
            ...
            >
              ...
              <div style={{ fontSize: '12px', color: '#64748b' }}>
                ...
                <p style={{ margin: '2px 0' }}>Completed: {agent.completedDeals}</p>
          ...
          {selectedAgent ? (
            <>
              <h2 style={{ margin: '0 0 16px 0', fontSize: '20px', fontWeight: '700', color: '#1e293b' }}>
                Deals by {agents.find(a => a.agentId === selectedAgent)?.agentName}
              </h2>
              {agentDeals.length > 0 ? (
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                        <div>
                          ...
                          <p style={{ margin: '0', fontSize: '12px', color: '#64748b' }}>
                            Price: ₹{deal.agreedPrice?.toLocaleString('en-IN') || 'N/A'}
                          </p>
                        </div>
                        <span style={{
